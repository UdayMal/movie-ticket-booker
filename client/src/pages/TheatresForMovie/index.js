...
function TheatresForMovie() {
  ...
  const getTheatres = async () => {
    try {
      ...
      dispatch(HideLoading());
      message.error("Error fetching theatre data");
    }
  };

  useEffect(() => {
    getData();
  }, [params.id]); // ✅ Call only when movie ID changes

  useEffect(() => {
    getTheatres();
  }, [date, params.id]); // ✅ Fetch theatres when date or movie ID changes
  ...
  return movie ? (
    <div>
      ...
      {theatres.length > 0 ? (
        ...
        <h2>No theatres available for this movie on selected date.</h2>
      )}
    </div>
  ) : (
    <h1>Loading...</h1>
